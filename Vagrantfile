Vagrant.configure("2") do |config|
  ...
  config.vm.define "cachingproxy" do |cachingproxy|
    cachingproxy.vm.box = "eurolinux-vagrant/scientific-linux-7"
    cachingproxy.vm.network "private_network", ip: "192.168.60.202"
  	cachingproxy.vm.synced_folder "tmp/", "/tmp/shfs",mount_options:["dmode=777,fmode=777"]
    cachingproxy.vm.provision :ansible do |ansible|
      ansible.playbook= "playbook.yml"
    end
  end
